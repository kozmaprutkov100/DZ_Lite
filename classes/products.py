...
class Chemicals(Product):
    """
    Класс, представляющий домашнюю химию, наследующий класс Product.
    """
    def __init__(self, name, price, brand, warranty_period):
        super().__init__(name, price)
        self.brand = brand
    ...
    def get_details(self):
        return f"Домашняя химия: {self.name}, Бренд: {self.brand}, Цена: {self.price} руб, Срок годности: {self.warranty_period} лет"
